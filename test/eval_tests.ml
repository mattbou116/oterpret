...
let test_input input =
  let f (x, y) =
    ...
    let o, _ = Eval.eval ast empty_env in
    if y <> o
    then (
      let got = Format.asprintf "%a@\n" Object.pp_obj o in
      let exp = Format.asprintf "%a@\n" Object.pp_obj y in
      assert_failure
        ("Eval failed on: " ^ x ^ "; got: " ^ got ^ "; exp: " ^ exp ^ "\n"))
    else ()
  in
  List.iter f input
;;
...
let test_integer_literal _ =
  let open Object in
  let input = [ "5", INT_OBJ 5; "10", INT_OBJ 10 ] in
  test_input input
;;

let test_boolean_literal _ =
  let open Object in
  let input = [ "true", BOOL_OBJ true; "false", BOOL_OBJ false ] in
  test_input input
;;
...
let test_null_if_expression _ =
  let open Object in
  let input = [ "if (false) { 10 }", NULL; "if (1 > 2) { 10 }", NULL ] in
  test_input input
;;
...
let test_single_return_statement _ =
  let open Object in
  let input =
    [ "return 10;", INT_OBJ 10
    ; "return 10; 9;", INT_OBJ 10
    ; "return 2 * 5; 9;", INT_OBJ 10
    ; "9; return 2 * 5; 9;", INT_OBJ 10
    ; "if (10 > 1) { return 10; }", INT_OBJ 10
    ]
  in
  test_input input
;;
...
let test_nested_return_statement _ =
  ...
  in
  let input =
    [ one, INT_OBJ 10; two, INT_OBJ 10; three, INT_OBJ 20; four, INT_OBJ 20 ]
  in
  test_input input
;;
