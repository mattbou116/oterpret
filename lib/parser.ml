...
let cur_token_is (p : parser) (t : token) = p.cur_token = t
let peek_token_is (p : parser) (t : token) = p.peek_token = t
  ...
  let rec aux (parser : parser) (ast : ast) : ast =
    ...
    | _ ->
      (match parse_statement parser with
       | Ok (parser, stmt) ->
         aux (next_token parser) { statements = stmt :: ast.statements }
...
and parse_let_statement (p : parser) =
  ...
  | IDENT i ->
    ...
    if p.peek_token = SEMICOLON
    then Ok (next_token p, Let { name; value })
    else Ok (p, Let { name; value })
  | _ -> Error (next_token p)

(* parser -> (parser * statement, parser) result *)
and parse_return_statement (p : parser) =
  Result.bind (parse_expression LOWEST p) @@ fun (p, expr) -> Ok (p, Return expr)
...
and parse_expression_statement (p : parser) =
  ...
  @@ fun (p, expr) ->
  if p.cur_token = SEMICOLON
  then Ok (next_token p, ExpressionStatement expr)
...
and parse_boolean_literal (b : bool) = BooleanLiteral { value = b }

(* parser -> (parser * expression, parser) result *)
and parse_grouped_expression (p : parser) =
  (* Printf.printf "ENTERED PARSE GROUP EXPRESSION\n"; *)
  Result.bind (parse_expression LOWEST @@ next_token p)
  @@ fun (p, expr) -> Result.bind (expect_peek p RPAREN) @@ fun p -> Ok (p, expr)
...
and parse_if_expression (p : parser) =
  ...
  @@ fun (p, consequence) ->
  if p.peek_token != ELSE
  then Ok (p, If { condition; consequence; alternative = NoStatement })
  else
    Result.bind (parse_block_statement @@ next_token @@ next_token p)
    @@ fun (p, alternative) -> Ok (p, If { condition; consequence; alternative })
...
and parse_block_statement (p : parser) =
  ...
  let rec aux (p : parser) (acc : statement list) =
    ...
    | RBRACE ->
      if List.length acc = 0 then Ok (p, NoStatement :: acc) else Ok (p, acc)
    | _ ->
      Result.bind (parse_statement p)
      @@ fun (p, stmt) -> aux (next_token p) (stmt :: acc)
  in
  Result.bind (aux p []) @@ fun (p, stmts) -> Ok (p, Block (List.rev stmts))
...
and parse_function_args (p : parser) =
  let rec aux (p : parser) (acc : identifier list) =
    ...
    | _ -> Error (next_token p)
  in
  Result.bind (aux p []) @@ fun (p, idents) -> Ok (p, List.rev idents)
...
and parse_call_arguments (p : parser) =
  ...
  in
  Result.bind (aux p []) @@ fun (p, exprs) -> Ok (p, List.rev exprs)
...
and order_by_prec (p : parser) (prec : prec) (left : expression) =
  ...
  if p.peek_token != SEMICOLON && prec < prec_of_token p.peek_token
  then (
    match infix_fn (next_token p) left with
...
and parse_expression (prec : prec) (p : parser) =
  ...
  Result.bind (prefix_fn p) @@ fun (p, left) -> order_by_prec p prec left
;;
