...
module Lexer = struct
  ...
  let update_lexer lexer c : t =
    { lexer with ch = c; pos = lexer.read_pos; read_pos = lexer.read_pos + 1 }
  ;;

  let read_char lexer : t =
    if lexer.read_pos >= String.length lexer.input
    then update_lexer lexer '\000'
    else update_lexer lexer @@ String.get lexer.input lexer.read_pos
  ;;
  ...
  let rec read_identifier (lexer : t) (acc : string) : t * string =
    ...
    then read_identifier lexer (acc ^ Char.escaped lexer.ch)
    else lexer, acc
  ;;
  ...
  let rec read_number (lexer : t) (acc : string) : t * string =
    ...
    then read_number lexer (acc ^ Char.escaped lexer.ch)
    else lexer, acc
  ;;
  ...
  let next_token lexer : t * Token.t =
    ...
    match lexer.ch with
    (* operators *)
    | '=' when peek_char lexer = '=' -> read_char @@ read_char lexer, EQ
    | '=' -> read_char lexer, ASSIGN
    | '+' -> read_char lexer, PLUS
    | '-' -> read_char lexer, MINUS
    | '!' when peek_char lexer = '=' -> read_char @@ read_char lexer, NOT_EQ
